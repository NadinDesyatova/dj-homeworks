...
def books_on_required_day(request, required_pub_date):
    ...
    template = 'books/books_list.html'
    books_objects = Book.objects.filter(pub_date=required_pub_date)
    books_next = Book.objects.filter(pub_date__gt=required_pub_date).order_by('pub_date').first()
    books_previous = Book.objects.filter(pub_date__lt=required_pub_date).order_by('-pub_date').first()
